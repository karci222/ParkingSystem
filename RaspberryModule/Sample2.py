...
while True:
   try:
	response = updater.get(updater.base_url + "/data/SPACES?where=REGION_NUM%3D0", 		updater.generalHeaders)
	print response

	data = response.json()
	arr = {}
	arr2 = [0, 0, 0, 0]
	con = sqlite3.connect('/home/pi/testDb1.db', timeout=10)
	...
	print arr2
	print arr
	cursor = updater.getDataFromTable("0")
	for row in cursor:
		print row [0] #ID
